...
const allowedOrigins = config['cors allowed origins'] || [];
...
function authCheck(req, res, next) {
    ...
    // Print incoming and configured server_url for debugging
    console.log('Incoming server_url:', server);
    console.log('Configured server_url:', config['linked form and data server']['server url']);
    account
        .get(server)
        .then((account) => {
            console.log('Account lookup result (then):', account);
            if (!account) {
                const error = new Error('Account not found for this server_url.');
                error.status = 403;
                return next(error);
            }
...
function _setQuotaUsed(req, res, next) {
    if (!req.app.get('account lib')) {
        ...
        console.log('req.account in _setQuotaUsed:', req.account);
        surveyModel
