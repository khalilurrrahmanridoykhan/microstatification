...
function getXFormInfo(survey) {
    ...
    const formListUrl = getFormListUrl(
        survey.openRosaServer,
        survey.openRosaId,
        survey.customParam
    );
    console.log('FormList URL:', formListUrl);
    return _request({
        url: formListUrl,
    ...
    return new Promise((resolve, reject) => {
        ...
        request[method](options, (error, response, body) => {
            if (error) {
                ...
                console.error('401 Unauthorized:', body); // Add this
                reject(error);
            } else if (
                ...
                error.status = response.statusCode;
                console.error('Request failed:', response.statusCode, body); // Add this
                reject(error);
            } else if (method === 'head') {
                resolve(response);
            } else {
