...
const Option = ({
  ...
  const handleFileChange = async (e) => {
    ...
    // Process options to ensure unique names
    const processedOptions = options.map((label, idx) => {
      let baseName = label.toLowerCase().replace(/[^a-z0-9-._:]/g, "_");
      let name = baseName;
      let suffix = 2;

      // Check for duplicates in already processed options
      while (processedOptions.some((opt, i) => i < idx && opt.name === name)) {
        name = `${baseName}_${suffix}`;
        suffix++;
      }
    ...
    if (typeof onBulkOptions === "function") {
      onBulkOptions(processedOptions);
    }
  };
