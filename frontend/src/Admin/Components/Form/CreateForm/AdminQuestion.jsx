...
import Option from './AdminOption';
import SubQuestion from './AdminSubQuestion';
...
const Question = ({ question, index, moveQuestion, handleQuestionChange, handleDeleteQuestion, toggleSettings, openSettings, handleAddOption, handleOptionChange, handleAddSubQuestion, handleSubQuestionChange, handleDeleteSubQuestion, handleShowModal, errors, allQuestions = [], isEditing, projectId, formId }) => {
  const ref = React.useRef(null);
  const [conditions, setConditions] = useState(question.conditions || [{ question: '', condition: '', value: '' }]);
  const [conditionLogic, setConditionLogic] = useState(question.conditionLogic || 'and');
  ...
  const addCondition = () => {
    const newConditions = [...conditions, { question: '', condition: '', value: '' }];
    setConditions(newConditions);
    handleQuestionChange(index, 'conditions', newConditions);
  };
  ...
  const displayType = question.type.startsWith('select_one') ? 'select_one' : question.type.startsWith('select_multiple') ? 'select_multiple' : question.type;

  console.log(isEditing ? "Editing mode" : "Creating mode");
  const questionsToDisplay = isEditing ? fetchedQuestions : allQuestions;

  return (
    <div ref={ref} style={{ opacity,border,backgroundColor,cursor }} className="mb-2 border-2 rounded p-2">
      ...
      > <MdFormatListBulletedAdd/> <span className='m-0 text-[12px]'>{displayType? displayType: "Select Types"}</span></button> 
      <input
        type="text"
    ...
    <CreateFormSettingBtn index={index} toggleSettings={toggleSettings} handleDeleteQuestion={handleDeleteQuestion} />
      ...
      {(question.type.startsWith('select_one') || question.type.startsWith('select_multiple')) && (
        <div className="mb-3 bg-blue-200 p-2">
      ...
      <Collapse in={openSettings[index]}>
        <div className="mt-3">
          <Tabs defaultActiveKey="questionOptions" id={`question-tabs-${index}`}>
            ...
            <Tab eventKey="skipLogic" title="Skip Logic">
              <div className="mt-3">
                <Tabs defaultActiveKey="addCondition" id={`skip-logic-tabs-${index}`}>
                    ...
                    <div className="mt-3">
                            ...
                            <tr key={conditionIndex}>
                              <td>
                                <Form.Select
                                  ...
                                  onChange={(e) => handleConditionChange(conditionIndex, 'question', e.target.value)}
                                >
                                  <option value="">Select a question</option>
                                  {questionsToDisplay.slice(0, index).map((q, qIndex) => (
                                    <option key={qIndex} value={q.name}>{q.label}</option>
                                  ))}
                                </Form.Select>
                              </td>
                              <td>
                                {condition.question && (['text'].includes(questionsToDisplay.find(q => q.name === condition.question)?.type) || questionsToDisplay.find(q => q.name === condition.question)?.type.startsWith('select_one') || questionsToDisplay.find(q => q.name === condition.question)?.type.startsWith('select_multiple')) && (
                                  <Form.Select
                                    value={condition.condition}
                                    onChange={(e) => handleConditionChange(conditionIndex, 'condition', e.target.value)}
                                  >
                                    <option value="">Select a condition</option>
                                    <option value="was_answered">Was Answered</option>
                                    <option value="was_not_answered">Was not Answered</option>
                                    <option value="=">=</option>
                                    <option value="!=">!=</option>
                                  </Form.Select>
                                )}
                                {condition.question && ['integer', 'decimal'].includes(questionsToDisplay.find(q => q.name === condition.question)?.type) && (
                                  <Form.Select
                                    value={condition.condition}
                                    onChange={(e) => handleConditionChange(conditionIndex, 'condition', e.target.value)}
                                  >
                                ...
                                {condition.question && ['date', 'time', 'datetime', 'geopoint', 'geotrace', 'geoshape', 'image', 'audio', 'video', 'note', 'barcode', 'acknowledge', 'rating', 'range', 'file'].includes(questionsToDisplay.find(q => q.name === condition.question)?.type) && (
                                  <Form.Select
                                    value={condition.condition}
                                    onChange={(e) => handleConditionChange(conditionIndex, 'condition', e.target.value)}
                                  >
                              ...
                              <td>
                                {['=', '!=', '>', '<', '>=', '<='].includes(condition.condition) ? (
                                  questionsToDisplay.find(q => q.name === condition.question)?.type.startsWith('select_one') || questionsToDisplay.find(q => q.name === condition.question)?.type.startsWith('select_multiple') ? (
                                    <Form.Select
                                      value={condition.value}
                                      onChange={(e) => handleConditionChange(conditionIndex, 'value', e.target.value)}
                                    >
                                      <option value="">Select a value</option>
                                      {questionsToDisplay.find(q => q.name === condition.question)?.options.map((option, optionIndex) => (
                                        <option key={optionIndex} value={option.name}>{option.label}</option>
                                      ))}
                                    </Form.Select>
                                  ) : (
                                    <Form.Control
                              ...
                              <td>
                                <Button variant="danger" onClick={() => deleteCondition(conditionIndex)}>
                                  <FaTrash />
                                </Button>
                              </td>
                            </tr>
                      ...
                      <Form.Select
                        ...
                        onChange={(e) => {
                          ...
                          handleQuestionChange(index, 'conditionLogic', e.target.value);
                        }}
                      >
                        <option value="and">Previous responses should match all of these conditions.</option>
                        <option value="or">Previous responses should match any of these conditions.</option>
                      </Form.Select>
                      <br></br>
                      <Button variant="secondary" onClick={addCondition}>Add Condition</Button>
                      <Button variant="primary" onClick={saveConditionSkipLogic}>Save Condition Skip Logic</Button>
                    </div>
                  </Tab>
                  <Tab eventKey="manualSkipLogic" title="Manually enter your skip logic in XLSForm code">
                    <div className="mt-3">
                      <Form>
                        <Form.Group controlId="manualSkipLogic">
                          ...
                          <Form.Control
                            ...
                            onChange={(e) => handleQuestionChange(index, 'relevant', e.target.value)}
                          />
                        </Form.Group>
                        <Button variant="primary" onClick={() => saveManualSkipLogic(question.relevant)}>Save Manual Skip Logic</Button>
                      </Form>
                    </div>
                  </Tab>
                </Tabs>
...
function CreateFormSettingBtn({index,toggleSettings,handleDeleteQuestion}) {
  return (
    <div>
    <div className="flex md:flex-col justify-start items-center gap-2 text-sm md:mt-2 m-1" >
         <button type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-1 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" onClick={() => toggleSettings(index)}>
           <CiSettings className='w-6 h-6 inline-block text-white'/>
         </button>
         <button type="button" className=" focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-1 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" onClick={() => handleDeleteQuestion(index)}><RiDeleteBin5Line className='w-6 h-6 inline-block text-white' /></button>
         </div>
    </div>
  )
}
