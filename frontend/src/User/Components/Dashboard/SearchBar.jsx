...
const SearchBar = () => {
  ...
  useEffect(() => {
    ...
    const fetchForms = async () => {
      try {
        ...
        const formIds = userResponse.data.profile.forms || []; // e.g., [1, 2]

        // Fetch each form by ID
        ...
        setForms(allForms);
        console.log("allForms data:", allForms);
  ...
  useEffect(() => {
    ...
    if (selectedItem.type === 'form') {
      navigate(`/projects/${selectedItem.project}/forms/${selectedItem.id}`);
      setSearchTerm('');
    }
  }, [selectedItem]);
  ...
  return (
    <div className="hidden md:flex items-center relative w-[80%] leading-7">
      ...
      <input
        type="search"
        placeholder="Search for specific form"
