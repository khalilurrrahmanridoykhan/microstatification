  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const assignedOrgIds = freshUser.profile?.organizations || [];

        // 2. Get all projects and orgs
        const [allProjectsRes, allOrgsRes] = await Promise.all([
        ...
        const allOrganizations = allOrgsRes.data;
        ...
        // 4. Filter relevant organizations
        const assignedOrgs = allOrganizations.filter(org => assignedOrgIds.includes(org.id));

        // 5. Update state
        setProjects(uniqueProjects);
        setOrganizations(assignedOrgs);
      } catch (err) {
