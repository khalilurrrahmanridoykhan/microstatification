...
class SelectOneQuestion extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (filteredChoices.isNotEmpty &&
        currentValue != null &&
        !filteredChoices.any(
          (choice) => choice['name'].toString() == currentValue,
        )) {
      effectiveValue = null; // Clear invalid selection
      WidgetsBinding.instance.addPostFrameCallback((_) {
        onChanged(null); // Reset invalid value
        debugPrint('Cleared invalid currentValue for $xpath: $currentValue');
      });
    ...
    return FormField<String>(
      validator: (value) {
        if (validationQuestion() &&
            (answers[xpath] == null || answers[xpath].isEmpty)) {
        ...
        if (filterEnabled &&
          ...
          if (filterAnswer != null) {
            ...
            if (!allowedOptions.contains(value)) {
              debugPrint(
                'Validation failed for $xpath: Selected value $value not in allowed options $allowedOptions for filterAnswer $filterAnswer',
              );
              return 'Selected option is not valid for the current filter';
            }
          } else {
            // If no filter answer, no options should be available, so any value is invalid
            debugPrint(
              'Validation failed for $xpath: No filter answer, value $value is invalid',
            );
