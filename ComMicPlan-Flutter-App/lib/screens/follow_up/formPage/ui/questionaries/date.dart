...
class _DateQuestionState extends State<DateQuestion> {
  ...
  void initState() {
    ...
    // ✅ If read-only with default, call onChanged immediately
  ...
  String _formatDisplayText() {
    ...
    if (widget.appearance == 'month-year') {
      ...
      return _currentDate!.toLocal().toString().split(' ')[0];
    }
  }
  ...
  Widget build(BuildContext context) {
    if (_formatValue(_currentDate) !=
        (widget.currentValue ?? widget.defaultValue)) {
      _initializeCurrentDate();
    }

    return FormField<String>(
      initialValue: widget.currentValue ?? widget.defaultValue,
      validator: (value) {
