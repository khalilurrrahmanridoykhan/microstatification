...
import 'package:gmgi_project/bloc/follow_up/saved_answer/bloc/saved_answer_bloc.dart';
import 'package:gmgi_project/models/answer.dart';
import 'package:gmgi_project/screens/follow_up/finalized_single_answer.dart/single_answer.dart';
          ...
          body: BlocConsumer<SavedAnswerBloc, SavedAnswerState>(
            ...
            builder: (context, state) {
              if (state is SavedAnswerLoadingState) {
                ...
                // Initialize filter questions from the first answer (assuming all forms have similar structure)
                if (allAnswers.isNotEmpty && filterQuestions.isEmpty) {
                  // Get the first Answer object from the first map entry
                  final firstAnswer = allAnswers.first.values.first;

                  // Use a simple approach for saved answers filtering since LocationFilterUtils may not work with Hive data
                  // Extract location-related questions manually
                  ...
                  for (var questionName in locationQuestions) {
                    if (firstAnswer.answers.containsKey(questionName)) {
                      // Get unique values for this question from all answers
                      final uniqueValues = <String>{};
                      for (var answerEntry in allAnswers) {
                        final answer = answerEntry.values.first;
                        final value = answer.answers[questionName];
                      ...
                      filterQuestions.add({
                        'name': questionName,
                        'options': uniqueValues.toList()..sort(),
                      });
                    }
                  }

                  // Initialize map with null values
                  for (var q in filterQuestions) {
                    _selectedFilters[q['name']] = null;
                  }

                  print(
                    'DEBUG: Found ${filterQuestions.length} filter questions for saved answers: ${filterQuestions.map((q) => q['name']).toList()}',
                  );
                  print('DEBUG: Initialized filters: $_selectedFilters');
                }

                // Apply location filters first
                List<Map<String, Answer>> locationFilteredAnswers = allAnswers;
                if (filterQuestions.isNotEmpty &&
                    _selectedFilters.values.any((v) => v != null)) {
                  locationFilteredAnswers =
                      allAnswers.where((answerEntry) {
                        final answer = answerEntry.values.first;

                        // Check if this answer matches the selected filters
                        for (var filterName in _selectedFilters.keys) {
                          final selectedValue = _selectedFilters[filterName];
                          if (selectedValue != null) {
                            final answerValue = answer.answers[filterName];
                                  ...
                                  return Padding(
                                    ...
                                    child: SizedBox(
                                      ...
                                      child: DropdownButtonFormField<String>(
                                        value: _selectedFilters[questionName],
                                        ...
                                        onChanged: (value) {
                                          setState(() {
                                            _selectedFilters[questionName] =
                                                value;
                                            print(
                                              'DEBUG: Updated saved answer filter $questionName = $value',
                                            );
                                            print(
                                              'DEBUG: Current filters: $_selectedFilters',
                                            );
                                          });
                                        },
                                      ),
