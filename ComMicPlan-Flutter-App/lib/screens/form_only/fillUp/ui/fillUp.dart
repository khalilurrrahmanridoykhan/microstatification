...
import 'package:gmgi_project/bloc/form_only/fillUp/bloc/fill_up_bloc.dart';
import 'package:gmgi_project/screens/form_only/fillUp/ui/fillUpTile.dart';
...
class _FillUpState extends State<FillUp> {
  ...
  @override
  void dispose() {
    _fillUpBloc.close(); // Close the BLoC to prevent memory leaks
    super.dispose();
  }
        ...
        body: BlocConsumer<FillUpBloc, FillUpState>(
          ...
          builder: (context, state) {
            ...
            if (state is FillUpLoadedState) {
              // Check if state.form is null or empty
              ...
              final filteredForms =
                  allForms.where((form) {
                    // Safely access form['name']
                    final name = form['name']?.toString();
                    if (name == null)
                      return false; // Skip forms with null or invalid name
