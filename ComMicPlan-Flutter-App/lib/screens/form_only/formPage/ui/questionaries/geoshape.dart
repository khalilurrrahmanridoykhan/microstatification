...
class GeoshapeQuestion extends StatefulWidget {
  final String label;
  final String xpath;
  final String kuid;
  final String? currentValue; // Current stored geoshape
  final String? defaultValue; // Optional default geoshape
  final Function(String?) onChanged;
  final bool readOnly; // Disable button if true

  const GeoshapeQuestion({
    super.key,
    required this.label,
    required this.xpath,
    required this.kuid,
  ...
  @override
  State<GeoshapeQuestion> createState() => _GeoshapeQuestionState();
}

class _GeoshapeQuestionState extends State<GeoshapeQuestion> {
  bool _loading = false;
        ...
        : Column(
          ...
          children: [
            ...
            displayValue.isNotEmpty
                ...
                : const Text('No shape selected'),
          ],
        );
  }
}

class GeoshapeMapScreen extends StatefulWidget {
  final String label;
  final String xpath;
  final String kuid;
  ...
  @override
  State<GeoshapeMapScreen> createState() => _GeoshapeMapScreenState();
}

class _GeoshapeMapScreenState extends State<GeoshapeMapScreen> {
  ...
  void initState() {
    ...
    if (widget.currentValue != null && widget.currentValue!.isNotEmpty) {
      _points = _parseGeoshape(widget.currentValue!);
    }
    _getUserLocation();
    if (!widget.readOnly) {
  ...
  List<LatLng> _parseGeoshape(String value) {
    List<LatLng> points = [];
    try {
      ...
      debugPrint('Error parsing geoshape: $e');
    }
    return points;
  }

  String _pointsToGeoshapeString() {
    if (_points.isEmpty) return '';
    final pointsWithClosure = [..._points, _points.first];
    return pointsWithClosure
        .map((p) => '${p.latitude} ${p.longitude} 0.0 0.0')
        .join(';');
  }
    ...
    try {
      ...
      setState(() {
        if (_points.isEmpty || _points.last != point) {
          _points.add(point);
          widget.onChanged(_pointsToGeoshapeString());
          _fitBounds();
        }
      });
    } catch (e) {
  ...
  void _clearAllPoints() {
    ...
    try {
      ...
      debugPrint('Error clearing polygon: $e');
    }
  }
                  ...
                  Text(
                    _locationPermissionDenied
                        ? 'Location permission denied. ${widget.readOnly
                            ...
                            : 'Tap to add points.'}'
                        : widget.readOnly
                        ? 'Read-only mode. Displaying existing shape.'
                        : _isAutomaticMode
                        ? 'Automatic mode: Adding points every $_selectedDuration seconds. ${_points.isNotEmpty ? "Points: ${_points.length}" : ""}'
                        : _currentLocation != null
                        ? 'Tap to add points to form a polygon. ${_points.isNotEmpty ? "Points: ${_points.length}" : ""}'
